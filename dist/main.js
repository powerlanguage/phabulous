...
acc.push(trackedAction);}});return acc;},[]);}},{key:"formatActions",value:function formatActions(actions){if(actions&&actions.length>0){return actions.join(", ").toLowerCase().replace(/,(?=[^,]*$)/," and");}else{return"";}}},{key:"convertRecipientsStringToArray",value:function convertRecipientsStringToArray(phabString){return phabString.trim().split(",").map(function(item){return item.trim();});}},{key:"shouldIgnoreAnyRecipient",value:function shouldIgnoreAnyRecipient(recipients){for(var i=0;i<IGNORED.length;i++){if(recipients.indexOf(IGNORED[i])>-1){return true;}}return false;}},{key:"isBuildMessage",get:function get(){return this.sender==="jenkins"||this.sender==="Phabricator"&&this.body.indexOf("Harbormaster")>-1;}}]);return PhabricatorEmail;}();
